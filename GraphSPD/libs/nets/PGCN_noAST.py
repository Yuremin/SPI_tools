...
def PGCNTrain(model, trainloader, optimizer, criterion):
    ...
    for data in trainloader:  # Iterate in batches over the training dataset.
        ...
        data.y = one_hot(data.y, num_classes=2)
        ...
        accuracy.update(preds, data.y)
        precision.update(preds, data.y)
        recall.update(preds, data.y)
    ...
    return model, lossTrain, final_accuracy.item(), final_precision.item(), final_recall.item()
